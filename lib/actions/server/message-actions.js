...
class ActionsMessageActions
{
    ...
    async executeMessageActions(client, data, room, playerSchema)
    {
        ...
        if(GameConst.SHOOT === data.act){
            return this.executeShootAction(playerSchema, data, room);
        }
        if(GameConst.RELOAD === data.act){
            return this.executeReloadAction(playerSchema);
        }
    ...
    executeShootAction(playerSchema, data, room)
    {
        if(!playerSchema.weaponManager){
            Logger.error('WeaponManager not available for player.', playerSchema.broadcastKey);
            return false;
        }
        let angle = sc.get(data, 'angle', 0);
        return playerSchema.weaponManager.shoot(angle, room);
    }

    executeReloadAction(playerSchema)
    {
        if(!playerSchema.weaponManager){
            Logger.error('WeaponManager not available for player.', playerSchema.broadcastKey);
            return false;
        }
        return playerSchema.weaponManager.reload();
    }
