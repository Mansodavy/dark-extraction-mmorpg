...
class SceneDynamic extends Scene
{
    ...
    setupKeyboardAndPointerEvents()
    {
        ...
        this.input.on('pointermove', (pointer) => {
            return this.executePointerMoveAction(pointer);
        });
    }
    ...
    executeKeyDownBehavior(event)
    {
        ...
        if(Input.Keyboard.KeyCodes.R === event.keyCode){
            if(!this.player){
                return false;
            }
            this.player.reload();
    ...
    executePointerDownAction(pointer, currentlyOver)
    {
        ...
        if(pointer.leftButtonDown()){
            ...
            this.player.shoot(this.playerMouseAngle);
            return true;
        }
    ...
    executePointerMoveAction(pointer)
    {
        if(!this.player || !this.player.players[this.player.playerId]){
            return false;
        }
        let playerSprite = this.player.players[this.player.playerId];
        let angle = Phaser.Math.Angle.Between(playerSprite.x, playerSprite.y, pointer.worldX, pointer.worldY);
        this.playerMouseAngle = angle;
        playerSprite.setRotation(angle);
        this.player.setMouseAngle(angle);
        return true;
    }
